...
// Function to fix specific problems
async function fixSpecificProblems() {
  try {
    // Connect to MongoDB
    ...
    for (const [title, data] of Object.entries(specificProblemData)) {
      try {
        ...
        // Update the problem with correct data
        await Problem.findByIdAndUpdate(problem._id, data);
        console.log(`✅ Fixed: ${title}`);
        fixedCount++;
        
      } catch (error) {
        console.error(`❌ Failed to fix ${title}:`, error.message);
      }
    }
    
    console.log(`\n🎉 Specific problem fixing completed!`);
...
module.exports = { fixSpecificProblems };
